...
class GameRepository(
     ...
     * для каждого игрока считаем игры, голы и передачи.
     */
    fun collectPlayerStats(): Map<String, PlayerStats> {
        val result = mutableMapOf<String, PlayerStats>()
        val files = listSavedGames()
        for (file in files) {
            try {
                val text = file.readText(Charsets.UTF_8)
                val root = JSONObject(text)
    ...
    /** Построение таблицы "Лучшие снайперы" (по голам). */
    ...
    /** Построение таблицы "Лучшие бомбардиры" (по очкам = голы + передачи). */
